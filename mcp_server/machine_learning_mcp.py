...
async def reviews_stars_correlation_test_tool(ItemName,reviews,stars):
    ...
    """
    print("reviews_stars_correlation_test_tool call ItemName:{} reviews:{} stars:{}")
...
async def exponential_moving_average(series, alpha, forecast_periods=1):
    """
    输入一个序列数据，计算指数加权移动平均法（EWMA）并预测未来值
    参数:
...
async def sales_predict_tool(ItemName,sales_data_list):
    ...
    :return:销量预测结果
    """
    print("sales_predict_tool input ItemName:{} sales_data_list:{}".format(ItemName, sales_data_list), flush=True)
    ...
    print("销量预测结果forecast:",forecast, flush=True)
    str_history=str(sales_data_list) #历史销量数据
    out = f"商品{ItemName}历史销量数据为:{str_history},经过指数平滑移动平均预测下一个月的销量为:{forecast}"
    print("sales_predict_tool output:{}".format(out))
    return out

def test_analysis_product_reviews_tool():
    async def main():
...
if __name__ == "__main__":
    ...
    # nohup python  mcp_server/machine_learning_mcp.py > log.machine_learning_mcp 2>&1 & 

    # test
    # test_analysis_product_reviews_tool()
    # test_reviews_stars_correlation_test_function()
    # test_reviews_stars_correlation_test_tool()
    # test_sales_predict_tool()