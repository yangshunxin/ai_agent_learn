...
mcp = FastMCP(name='python_chart_mcp', instructions='Python绘图工具MCP',host="0.0.0.0",  port=9012)
...
async def run_python_script_tool(script_content: str):
    """
    执行用户提供的Python代码，并返回执行结果或生成的图片路径，注意：只能返回一张图片
...
if __name__ == "__main__":
    ...
    # nohup python  mcp_server/python_chart_mcp_debug.py > log.python_chart_mcp_debug 2>&1 & 
    #uv run python_chart_mcp_debug.py &
