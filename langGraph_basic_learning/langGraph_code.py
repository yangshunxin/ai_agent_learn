...
def save_graph_img():
    ...
    # plt.axis('off')
    # plt.show()
    png_path = './langGraph_basic_learning/graph_image.png'
...
def test_call_function():
    ...
    tools=[check_at_home]
    tool_node = ToolNode(tools) #为工具节点绑定参数
    # 人工构造一个 AIMessage, 调用工具前， 必须是 AIMessage
...
def test_call_function_and_check():
    # 创建状态图
    class State(TypedDict):
    ...
    png_path = './langGraph_basic_learning/graph_image_check.png'
    plt.imsave(png_path, img)
    # 调用工具节点
...
def test_choice_graph():
    ...
    # plt.show()
    img = mpimg.imread(BytesIO(graph_image), format='PNG')
    png_path = './langGraph_basic_learning/graph_image_choice.png'
    plt.imsave(png_path, img)
...
async def make_mcp_graph():
    ...
    async with client.session("my_mcp") as check_home_session:
        ...
        # plt.axis('off')
        # plt.show()
        png_path = './langGraph_basic_learning/graph_image_mcp.png'
        plt.imsave(png_path, img)
        yield graph
...
def test_react_agent():
    ...
    # plt.axis('off')
    # plt.show()
    png_path = './langGraph_basic_learning/graph_image_react.png'
    plt.imsave(png_path, img)
    result=agent_compiled_builder.invoke({"messages":"张三在不在家里"},config=thread_config)
