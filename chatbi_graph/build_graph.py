...
mcp_server_config = {
    "search_db_mcp":{
        "url": f"http://{server_url}:9004/mcp",
...
def should_continue_ywfl(state: BIState) -> Literal[END, "call_python_coder", "call_list_tables"]:
    ...
    if last_message.content=="纯python编码": #如果有工具调用则需要根据，业务分流的结果决策向哪里走
        return "call_python_coder"
    elif last_message.content=="业务数据查询分析":
...
async def make_graph():
    ...
    async with client.session("ywfl_mcp") as ywfl_session:
        ywfl_server_tools = await load_mcp_tools(ywfl_session) 
        tools.extend(ywfl_server_tools)
    
    async with client.session("machine_learning_mcp") as machine_learning_session:
    ...
    def call_python_coder(state: BIState):
        ...
        print("python写代码如下:",python_coder_result)
    ...
    def call_list_tables(state: BIState):
        """获取数据库信息节点"""
        print("xxxxxxxxxxxx", flush=True)
        tool_call_message = AIMessage(
            content="",
            tool_calls=[
                {
                    "name": "list_tables_tool",  # 你的工具名
                    "args": {},                  # 参数（无参数为空）
                    "id": "call_1"               # 随便一个唯一ID
                }
            ]
    ...
    def should_continue(state: BIState):
        ...
        if last_message.tool_calls:
            return "identify_intention_tool_node"
        return END
    
    workflow.add_conditional_edges(
        "call_identify_intention",
        should_continue,
        ["identify_intention_tool_node", END]
    )
    ...
    workflow.add_conditional_edges(
        "identify_intention_tool_node", # 从工具节点出发
        should_continue_ywfl, # 路由函数
        {
            "纯python编码": "call_python_coder",
            "业务数据查询分析": "call_list_tables"
        }
    )
    ...
    image_path = "./chatbi_graph/output/graph_multi_agent.png"
