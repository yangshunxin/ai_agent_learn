...
async def run_graph():
    ...
    async with make_graph() as graph:
        while True:
            ...
            whole_history.append(HumanMessage(user_input))
            if user_input.lower() in ['q', 'exit', 'quit']:
                ...
                # values：全量 state → 适合看整体、调试、最终结果
                # updates：仅增量 → 适合前端实时更新、减少数据传输        
                async for event in graph.astream({"messages":event_message_list},
                    ...
                    if isinstance(event['messages'][-1], HumanMessage):
                        continue 
                    # 如果当前事件不是工具类消息是回复则更新历史记录, 当前轮对话结束
                    if event["messages"][-1].content and isinstance(event["messages"][-1], AIMessage) and not event["messages"][-1].tool_calls:
                        ...
                        whole_history.append({event["messages"][-1]})
                        break
